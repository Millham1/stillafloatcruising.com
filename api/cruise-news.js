...
const TRUSTED_MAINSTREAM = [
  'cnn','reuters','associated press','ap news','cbs','nbc','fox','usa today','weather channel','travelandleisure','bloomberg'
];

const TIER_PRIORITY = { lifestyle: 300, impact: 200, industry: 100 };

function normalizeText(text='') { return text.replace(/&amp;/g, '&').replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/<[^>]*>/g, '').trim(); }
...
function categoryFor(tier='lifestyle') { if (tier === 'impact') return 'Cruise Impact'; if (tier === 'industry') return 'Industry Intelligence'; return 'Cruise Life'; }
...
function scoreStory(story) {
  ...
  const freshness = published ? Math.max(0, 24 - Math.floor((Date.now() - published) / 3600000)) : 0;
...
function dedupe(stories) {
  ...
  stories.forEach(story => {
    const key = story.title.toLowerCase().replace(/[^a-z0-9]/g, '').slice(0, 120);
    const current = seen.get(key);
    if (!current || story.score > current.score) seen.set(key, story);
  });
  return [...seen.values()];
}
...
function limitPerSource(stories, max = 2) {
  ...
  return stories.filter(story => {
    ...
    if (counts[story.source] >= max) return false;
    counts[story.source]++;
    return true;
  });
}
...
async function fetchNewsApi(search) {
  ...
  try {
    ...
    return (data.articles || []).map(article => normalizeStory({
      title: article.title,
...
async function fetchRssSource(source) {
  try {
    ...
    return items.map(match => {
      ...
      return normalizeStory({ title, description, link, source: source.name, publishedAt: pubDate, tier: source.name.includes('Industry') || source.name.includes('Seatrade') ? 'industry' : 'lifestyle' });
    });
  } catch { return []; }
}

async function getStories() {
  const mainstreamResults = await Promise.all(NEWSAPI_SEARCHES.map(fetchNewsApi));
  const rssResults = await Promise.all(RSS_SOURCES.map(fetchRssSource));
  const combined = [...mainstreamResults.flat(), ...rssResults.flat()];
  return limitPerSource(dedupe(combined).sort((a, b) => b.score - a.score), 2);
}

function homepageStories(stories) {
  const selected = limitPerSource(stories, 1);
  return selected.length >= 5 ? selected.slice(0,5) : stories.slice(0,5);
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({ ok: true, source: 'newsapi-editorial-engine', storyCount: stories.length, generatedAt: new Date().toISOString(), homepage: homepageStories(stories), stories });
  } catch (error) {
    return res.status(500).json({ ok: false, error: error.message, homepage: [], stories: [] });
  }
}
