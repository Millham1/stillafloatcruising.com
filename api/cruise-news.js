...
function isCruiseRelevant(story) {
  ...
  return CRUISE_TERMS.some(term => text.includes(term));
}
...
async function fetchRssSource(source) {
  try {
    ...
    return items.map(match => {
      ...
      const link = tagValue(itemXml, 'link') || tagValue(itemXml, 'guid');
...
async function fetchGNewsSearch(query, apiKey) {
  ...
  try {
    ...
    return articles.map(article => normalizeStory({
      title: article.title,
