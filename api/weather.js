...
function emojiFromOpenWeather(main = '', id = 0) {
  ...
  if (value.includes('cloud')) return id === 801 || id === 802 ? '⛅' : '☁️';
...
async function fetchOpenWeather(destination, apiKey) {
  const url = new URL('https://api.openweathermap.org/data/2.5/weather');
  url.searchParams.set('lat', destination.lat);
  url.searchParams.set('lon', destination.lon);
  url.searchParams.set('appid', apiKey);
  url.searchParams.set('units', 'imperial');

  const response = await fetch(url.toString());
  if (!response.ok) throw new Error(`OpenWeather failed for ${destination.slug}: ${response.status}`);
  ...
  return {
    ...
    emoji: emojiFromOpenWeather(weather.main, weather.id),
    condition: weather.main || '',
    forecastUrl: `forecast.html?place=${destination.slug}`
  };
}

async function fetchOpenMeteoCurrent(destination) {
  ...
  const response = await fetch(url.toString());
  if (!response.ok) throw new Error(`Open-Meteo failed for ${destination.slug}: ${response.status}`);
  ...
  return {
    ...
    emoji: emojiFromWeatherCode(code),
    condition: '',
    forecastUrl: `forecast.html?place=${destination.slug}`
  };
}
...
async function buildWeather() {
  const apiKey = process.env.weatherpage || process.env.WEATHERPAGE || process.env.OPENWEATHER_API_KEY;

  const cards = await Promise.all(
    DESTINATIONS.map(async destination => {
      try {
        return apiKey
          ? await fetchOpenWeather(destination, apiKey)
          : await fetchOpenMeteoCurrent(destination);
      } catch (error) {
        return await fetchOpenMeteoCurrent(destination);
      }
    })
  );
...
export default async function handler(req, res) {
  ...
  try {
    const cards = await buildWeather();
